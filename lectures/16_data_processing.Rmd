...
```{r}
# in R version 4, this is no longer a factor by default
...
Create a sample from a data frame by selecting from row numbers.

```{r}
haart[sample(nrow(haart), 10),]
```

Let's modify the HAART data set to contain an ID and repeated measures.

```{r}
newdat <- cbind(id = sample(100, nrow(haart), replace = TRUE), haart[,c('weight','last.visit')])
```

## Exercise

How could we sample by ID?
